...
//  From Tom:
  ...
  TQChessBrd = class(TGraphicControl)
  private
    ...
    _whiteOnTop,_whiteToMove,_boardlines, _animateMoves, _MoveWaitComputer: Boolean;
  ...
  public
   ...
   function SquareIsWhite(sq: Square):Boolean;

  published
   property MoveWaitComputer: boolean read _MoveWaitComputer write _MoveWaitComputer;
...
procedure Register;
begin
  RegisterComponents('Chess', [TQChessBrd]);
end;


{TQChessBrd}

constructor TQChessBrd.Create(AOwner: TComponent);
var
    p,q: pInt;
begin
    ...
    SetNewGame;
    //thread önceden burdaydý

end;


destructor TQChessBrd.destroy;
begin
    if(_customEngine=FALSE)and(Now<>nil) then
    begin
      while not Now.Terminated do
        Now.Terminate;
      //Now.Suspend;
      //Now.Free;
    end;                                
...
procedure TQChessBrd.TimerCallback(Sender: TObject);
begin
    WndProc;
end;

procedure TQChessBrd.MouseDown(Button: TMouseButton; Shift: TShiftState;X,Y: Integer);
var
    sq: Square;
const
     space: Char=' ';
...
procedure TQChessBrd.ThinkingComplete(Sender: TObject);
begin
  Now.Terminate;
  Now := nil;
...
procedure TQChessBrd.Animate (oldSq,newSq: Square; delay: Integer);
var
    x1,y1,x2,y2: Integer;
begin
    ...
    DragObject.HideDragImage;
    //list.HideDragImage;
    //list.EndDrag;
    //list.DragUnlock;
end;

procedure TQChessBrd.AnimateHorizontally(x1,x2,y,delay: Integer);
var
    x,v: Integer;
begin
    if (x2>x1) then
...
procedure TQChessBrd.AnimateVertically(y1,y2,x,delay: Integer);
var
    y,v: Integer;
begin
    if (y2>y1) then
...
procedure TQChessBrd.AnimateDiagonally(x1,y1,x2,y2,delay: Integer);
var
    y,v: Integer;
    x,step: Real;
begin
...
procedure TQChessBrd.DragOver(Source:TObject;
...
procedure TQChessBrd.DragDrop(Source:TObject ;X,Y: Integer);
begin
...
procedure TQChessBrd.DragCanceled;
begin
    list.HideDragImage;
    list.EndDrag;
    list.DragUnlock;
...
procedure TQChessBrd.MouseMove(Shift: TShiftState;X,Y: Integer);
var
    w: Integer;
begin
...
procedure TQChessBrd.MouseUp(Button:TMouseButton;Shift:TShiftState;
          X,Y: Integer);
begin
...
procedure TQChessBrd.Click;
begin
    inherited Click();
end;

procedure TQChessBrd.EndDrag(drop: Boolean);
begin
    inherited EndDrag(drop);
end;

procedure TQChessBrd.Promotion (Sender:TObject; oldSq,newSq: Square;
     var NewPiece: Char);
var
    i: String;
    r: Integer;
...
procedure TQChessBrd.DoPromotion (sq: Square);
begin
    if YPos(sq)=8 then
        begin
...
function TQChessBrd.Get_Position: String;
begin
     Result:=StrPas(@_position[1]);
end;

procedure  TQChessBrd.Set_ComputerPlaysBlack(plays: Boolean);
begin
    CancelThinking;
...
procedure  TQChessBrd.Set_ComputerPlaysWhite(plays: Boolean);
begin
    CancelThinking;
...
procedure TQChessBrd.Set_Position(pos:String);
var
    PieceStr: String;
    q,n,len: Integer;
begin
...
procedure TQChessBrd.Set_ResizeMinSize (size: Integer);
begin
    if (size<_ResizeMaxSize) then _ResizeMinsize:=size
        else _ResizeMinSize:=_ResizeMaxSize;
end;

procedure TQChessBrd.Set_ResizeMaxSize (size: Integer);
begin
...
procedure TQChessBrd.Set_SearchDepth (depth: Integer);
begin
...
procedure  TQChessBrd.Set_SizeOfSquare (size: Integer);
begin
    _SizeOfSquare:=size;
    Width:=8*_SizeOfSquare+2*_SizeOfBorder;
end;

procedure  TQChessBrd.Set_CurrentMove (moveno: Integer);
begin
    // Read-Only
end;

procedure  TQChessBrd.Set_EnPassant (sq: Square);
begin
...
procedure  TQChessBrd.Set_BoardLines (show: Boolean);
begin
    _boardLines:=show;
    Paint;
end;

function TQChessBrd.Get_Thinking: Boolean;
begin
    Result:=FALSE;
    if (Now<>nil)then
        Result:=Now.Thinking;
...
procedure TQChessBrd.Set_Thinking (thinking:Boolean);
begin
    // Read-Only
end;

procedure TQChessBrd.Set_ThinkingPriority (priority: TThreadPriority);
begin
    _ThinkingPriority:=priority;
end;

procedure  TQChessBrd.Set_DisplayCoords (cset: CoordSet);
begin
    _displayCoords:=cset;
    Paint;
end;

procedure  TQChessBrd.Set_CoordFont(f: TFont);
begin
    _coordfont.Assign(f);
    Paint;
end;

procedure  TQChessBrd.Set_CustomPieceSet(bmp: TBitmap);
begin
...
procedure  TQChessBrd.Set_CustomEngine (use: Boolean);
begin
     _customEngine:=use;
end;

procedure  TQChessBrd.Set_LineStyle (pen: TPen);
begin
    _lineStyle.Assign(pen);
    Paint;
end;

procedure  TQChessBrd.Set_SizeOfBorder (border: Integer);
begin
...
procedure TQChessBrd.Set_Version (str: String);
begin
    // Read-only
end;

procedure  TQChessBrd.Set_WhiteOnTop (wabove: Boolean);
begin
    _whiteOnTop:=wabove;
    Paint;
end;

procedure  TQChessBrd.Set_WhiteToMove (wmove: Boolean);
begin
    CancelThinking;
    _whiteToMove:=wmove;
end;

procedure  TQChessBrd.Set_DarkSquare(bmp: TBitmap);
begin
    _SquareDark.Assign(bmp);
    Paint;
end;

procedure  TQChessBrd.Set_LightSquare(bmp: TBitmap);
begin
    _SquareLight.Assign(bmp);
    Paint;
end;

procedure  TQChessBrd.Set_BorderBitmap(bmp: TBitmap);
begin
    _borderBitmap.Assign(bmp);
    Paint;
end;

procedure  TQChessBrd.Set_SquareColorDark(c: TColor);
begin
    _squareColorDark:=c;
    Paint;
end;

procedure  TQChessBrd.Set_SquareColorLight(c: TColor);
begin
    _squareColorLight:=c;
    Paint;
end;

procedure  TQChessBrd.Set_BorderColor(c: TColor);
begin
    _borderColor:=c;
    Paint;
end;
...
procedure TQChessBrd.DrawChessPiece (canvas: TCanvas; x,y: Integer; piece: Char);
var
    v,i,j: Integer;
    c: Char;
begin
...
procedure  TQChessBrd.UpdateChessBoard (oldpos: String);
var
    sq: Square;
begin
    for sq:=A8 to H1 do
...
procedure  TQChessBrd.DrawPiece (sq: Square; piece: Char);
var
    x,y,adj: Integer;
begin
...
procedure TQChessBrd.ClearSquare(sq: Square);
var
...
function TQChessBrd.WindowToSquare (x,y: Integer): Square;
var
    xn,yn,xv,yv: Integer;
begin
...
//
function TQChessBrd.BitmapExists (bmp: TBitmap) : Boolean;
begin
    Result:=((bmp<>nil)and(bmp.Width>0)and(bmp.Height>0));
end;

function TQChessBrd.BitmapIsValidPieceSet (bmp: TBitmap) : Boolean;
begin
    Result:=((bmp<>nil)and(bmp.Width>0)and(bmp.Height>0)and
             (bmp.Width*2=bmp.Height*3));
end;

procedure TQChessBrd.OrganizeBitmaps;
var
    Bmp: TBitmap;
    tmp, tmpmask: TBitmap;
    r,n: Integer;
...
procedure  TQChessBrd.DrawBorder;
var
    abc: String;
...
procedure  TQChessBrd.DrawPieces;
var
    sq: Square;
    len: Integer;
begin
...
procedure  TQChessBrd.DrawBoard;
var
...
procedure  TQChessBrd.DrawBoardLines;
var
    x: Integer;
begin
    if (_boardLines)then
...
procedure TQChessBrd.CancelThinking;
begin
    if(_CustomEngine=FALSE)and(Now<>nil)and(GameEnded=FALSE)then
    begin
        Now.StopThinkingNow:=TRUE;
        while (Now.Terminated=FALSE) do Now.Terminate;
        Now := nil;
    end; 
    StopThinking:=TRUE;
end;

procedure TQChessBrd.Think;
var
    oldsq, newsq: Square;
begin
...
procedure TQChessBrd.GetMoveList(var list:TStringList);
var
    c: Char;
    s: String;
    r,side: Integer;
...
function TQChessBrd.GetMove (moveno:Integer;whiteMoves: Boolean): MoveInfo;
begin
    temp.OldSquare:=None;
    temp.NewSquare:=None;
    temp.position:='init';
...
function TQChessBrd.ColorOfSquare (sq:Square): Integer;
begin
    Result:=Integer (((Integer(sq)-1) and 1)=(((Integer(sq)-1) shr 3) and 1));
end;

function  TQChessBrd.PieceToInt (piece: Char): Integer;
var
    i: String;
    r: Integer;
begin
...
function TQChessBrd.ColorOfPieceOnSquare (sq: Square): Integer;
begin
    Result:=NoPiece;
...
function TQChessBrd.ColorOfPiece (piece:Char): Integer;
var
    i: String;
    r: Integer;
begin
...
function TQChessBrd.XPos (sq: Square): Integer;
begin
    Result:=(1+(Integer(sq)-1) mod 8);
end;

// Returns YPos of a int according to ChessBoard coordinates (1-8)
function TQChessBrd.YPos (sq: Square): Integer;
begin
    Result:=8-((Integer(sq)-1) shr 3);
end;

function TQChessBrd.MouseToSquare (x,y: Integer): Square;
begin
...
function TQChessBrd.StringToSquare (str: String): Square;
begin
   str:=Lowercase(str);
   Result:=None;
   if (Length(str)=2)and
...
// writes into x and y
procedure TQChessBrd.SquareToCoords (sq: Square; var x,y: Integer);
var
    a,b: Integer;
begin
    a:=XPos(sq)-1;
...
function TQChessBrd.SetupPosition (pos: MoveInfo; moveno: Integer; whiteMoves:Boolean): Boolean;
var
    i: String;
    r,n: Integer;
    Label BreakOut;
...
procedure  TQChessBrd.NewGame;
begin
    SetNewGame;
    Paint;
end;

function TQChessBrd.SquareIsWhite(sq: Square):Boolean;
begin
  Result := _position[Integer(sq)] <> LowerCase(_position[Integer(sq)]);
end;

procedure TQChessBrd.SetNewGame;
begin
    CancelThinking;
    _position:='_rnbqkbnrpppppppp                                PPPPPPPPRNBQKBNR';
...
function TQChessBrd.Move(oldSq,newSq: Square): Boolean;
begin
    CancelThinking;
    Result:=PerformMove(oldsq,newsq);
    stopThinking:=FALSE;
    Think;
    if _MoveWaitComputer then
    begin
      if ((csDesigning in ComponentState)=FALSE) then
        begin
            _MoveWaitComputer := false;
            Now:=TQChessThread.CreateSelf;
...
function TQChessBrd.PerformMove (oldsq, newsq: Square): Boolean;
var
...
function TQChessBrd.MoveBackward: Boolean;
begin
...
function TQChessBrd.MoveForward: Boolean;
begin
    if (_whiteToMove)then
...
function TQChessBrd.GotoMove (moveno: Integer; whiteMoves: Boolean): Boolean;
var
    oldpos: String;
begin
...
function  TQChessBrd.LegalMoveAvailable: Boolean;
var
    r,n: Square;
begin
    Result:=TRUE;
...
function TQChessBrd.CheckForThreefoldPosition: Boolean;
var
...
function TQChessBrd.MoveIsLegal (oldsq, newsq: Square): Boolean;
var
    piece: Char;
begin
    Result:=FALSE;
...
function TQChessBrd.BlackInCheckAfter(oldsq, newsq: Square): Boolean;
var
    v: Char;
    pos: String;
    x,y,r,res: Integer;
...
function TQChessBrd.WhiteInCheckAfter(oldsq, newsq: Square): Boolean;
var
    x,y,r,res: Integer;
    kingsq: Square;
    v: Char;
...
function TQChessBrd.CheckLegalPawnMove(oldsq,newsq: Square): Boolean;
var
    deltax: Integer;
begin
    Result:=TRUE;
...
function TQChessBrd.CheckLegalKnightMove(oldsq,newsq: Square): Boolean;
var
...
function  TQChessBrd.CheckLegalBishopMove(oldsq, newsq: Square): Boolean;
var
    x,y,r,n,m,p: Integer;
    v: Square;
begin
...
function TQChessBrd.CheckLegalRookMove(oldsq, newsq: Square): Boolean;
var
    x,y,r,n,p: Integer;
    v: Square;
begin
...
function TQChessBrd.CheckLegalQueenMove(oldsq, newsq: Square): Boolean;
begin
    Result:=CheckLegalBishopMove(oldsq,newsq);
...
function TQChessBrd.CheckLegalKingMove(oldsq,newsq: Square): Boolean;
var
    deltax,deltay: Integer;
begin
    Result:=TRUE;
...
procedure  TQChessBrd.InitializeBitmap;
begin
      Default.Handle:=LoadBitmap(hInstance, StrPCopy(@buf,SetAndrew40Str));
end;

//-------------------------------------------------------------------------------
// Implementation of TQChessThread (containing mainly Tom's Source)
//-------------------------------------------------------------------------------

constructor TQChessThread.CreateSelf;
begin
    InitValues;
...
procedure TQChessThread.Execute;
begin
...
procedure TQChessThread.ThinkAboutAMove;
var
    x,y,r,v,i: Integer;
    ch: Char;
begin
...
procedure TQChessThread.ThinkingFinished;
begin
    EndFunc(Self);
end;

// Synchronizer - Modifies VCL
procedure TQChessThread.PerformMove;
begin
...
procedure TQChessThread.IntCopy (dest,source: pInt; count: Integer);
var
   r: Integer;
begin
...
function TQChessThread.ColorOfPiece (sq: Square): Integer;
begin
...
procedure TQChessThread.sort_pv;
var
    i: Integer;
    m1,m2: Move_Bytes;
begin
...
// a cutoff.
procedure TQChessThread.sort(src: Integer);
var
   i:  Integer;
...
function TQChessThread.quiesce(alpha,beta: Integer): Integer;
var
    i,j,x: Integer;
    c,f: Boolean;
begin
...
function TQChessThread.search(alpha,beta,depth: Integer): Integer;
var
    i,j,x: Integer;
    c,f: Boolean;
begin
...
function TQChessThread.in_check(s: Integer): Boolean;
var
    i: Integer;
begin
    for i:=0 to 63 do
...
function TQChessThread.attack(sq,s: Integer ): Boolean;
var
    r,i,j,n: Integer;
begin
    for i:=0 to 63 do
...
// quiescence search.
procedure TQChessThread.gen_caps;
var
    r,i,j,n: Integer;
begin
    gen_end[ply]:=gen_begin[ply];
...
procedure TQChessThread.gen_push(src,dst,bits: Integer);
var
    g: pGenRec;
begin
...
procedure TQChessThread.gen_promote(src,dst,bits: Integer);
var
    i: Integer;
    g: pGenRec;
begin
...
// returns TRUE.
function TQChessThread.makemove(m: move_bytes): Boolean;
begin
...
procedure TQChessThread.takeback;
var
    m: move_bytes;
    src,dst: Integer;
begin
...
function TQChessThread.eval: Integer;
var
...
procedure TQChessThread.init_eval;
var
...
procedure TQChessThread.InitValues;
const
