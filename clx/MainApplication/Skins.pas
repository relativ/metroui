...
uses
  ...
  QDialogs, QStdCtrls, QComCtrls, QExtCtrls, JvSimpleXml, SkinBase,
  Configuration,{$IFDEF MSWINDOWS} Vcl.Imaging.pngimage, {$ENDIF}  ButtonEventList;
...
type
  ...
  public
    ...
    procedure DrawSkinMainForm(Canvas: TCanvas; DrawRect,SelectRect: TRect);
...
begin
  ...
  begin
    ...
    begin
      ...
      if modval > 0 then modval := 1 else modval := 0;
...
procedure TSkins.DrawSkinMainForm(Canvas: TCanvas; DrawRect,SelectRect: TRect);
var
  ...
  RecNo: integer;
begin
  BackgroundImage := Settings.SkinPath + '\screens\main_off.png';
  DownBackgroundImage := Settings.SkinPath + '\screens\main_down.png';
    ...
    begin
      ...
      begin
        ...
        begin
            ...
            if ButtonIndex = 0 then ButtonIndex := 6;
           // ShowMessage(IntToStr(RecNo)+'--'+IntToStr(ButtonIndex)+'--'+IntToStr(FNavigationPage * 6)+'---'+IntToStr((FNavigationPage + 1) * 6));
            ...
            iconPath := ExtractFilePath(ButtonEventList.EventList.FieldByName('libpath').AsString) + 'Skin\'+ButtonEventList.EventList.FieldByName('icon').AsString;
            ...
            GetButtonCaptionPosition(Settings.MenuButtonBaseName + IntToStr(ButtonIndex)).PluginButtonName := ButtonEventList.EventList.FieldByName('buttonname').AsString;
